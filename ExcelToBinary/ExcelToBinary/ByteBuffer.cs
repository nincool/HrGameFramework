...
//{
//    public virtual void Save(ByteBufferWrite buffer)
//    {
//        SaveAuto(buffer);
//    }
...
//    public void SaveAuto(ByteBufferWrite buffer)
//    {
...
//    private  void SavePropertyValue(PropertyInfo property, ByteBufferWrite buffer)
//    {
...
public class ByteBufferWrite
{
    ...
    public ByteBufferWrite(string strPath, bool bOpen, Encoding encoding)
    {
    ...
    public ByteBufferWrite(string strPath, bool bOpen)
    {
    ...
    public void WriteString(string strIn)
    {
        if (strIn == null)
        {
            WriteUShort(0);
...
public class ByteBufferRead
{
    ...
    public ByteBufferRead(byte[] bytes)
    {
        m_fsSource = new MemoryStream(bytes);
        mEncoding = Encoding.UTF8;
    }

    public ByteBufferRead(string strPath, Encoding encoding)
    {
        mEncoding = encoding;
        bool bRes = Open(strPath); 
        if (!bRes)
        {
            throw new Exception(String.Format("打开文件【{0}】失败！", strPath));
        }
    ...
    public string ReadString()
    {
        ...
        return mEncoding.GetString(buff);
    }
    ...
    public void ReadString(ref string strOut, UInt16 nSize)
    {
        ...
        if (nSize > 0)
        {
            ...
            {
                strOut = mEncoding.GetString(bytes);
            }
        }
    }

    public void ReadFloat(ref float nValue)
    {
        byte[] bytes = new byte[sizeof(float)];
        if (m_fsSource != null && (m_fsSource.Read(bytes, 0, sizeof(float)) == sizeof(float)))
            nValue = BitConverter.ToSingle(bytes, 0);
        else
            nValue = 0;
    }
